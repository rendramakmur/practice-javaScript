...
function ongkosKirim(pengirim) {
  let kotaTujuan = ['Depok', 'Jakarta', 'Bandung', 'Surabaya', 'Bali'];
  let output = [];
  ...
  for (let i = 0; i < pengirim.length; i++) {
    ...
    for (let j = 0; j < kotaTujuan.length; j++) {
      
      if (kotaTujuan[j] === pengirim[i].asal) {
        flag = true;
      }
      // console.log(pengirimTemp, flag);
      
      if (flag) {
        durasi++;
      }
      
      if (kotaTujuan[j] === pengirim[i].tujuan) {
        flag = false;
      }
      // Kalau satu kota
    ...
    if (pengirim[i].ekspedisi === 'NJE') {
      reportDelivery.tagihan = (9000 * durasi) - (9000 * durasi * 10/100);
    } else {
      reportDelivery.tagihan = 9000 * durasi;
    }

    output.push(reportDelivery);
  }
